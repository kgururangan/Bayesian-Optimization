...
%
nruns = 10; niter = 1e3; alph = 0.01; beta = 1.0; normalize = 2;
...
while flag_acq == 0 && it <= maxit
    ...
    % option 2
    Z = @(ind) (Mu(ind) - max(yT) - kappa)./Sig(ind);
    PI_acq(i1) = (Phi(Z(i1)).*Z(i1) + phi(Z(i1))).*Sig(i1);
    ...
    if flag_plot == 1
        ...
        colormap(cmap)
        hold on
        surf(s,s,Mu_rs+2*Sig_rs,'FaceAlpha',0.3,'EdgeColor','none')
        surf(s,s,Mu_rs-2*Sig_rs,'FaceAlpha',0.3,'EdgeColor','none')
        ...
        ll = legend([hh1,hh2],{'True Error','\mu Error'}); set(ll,'Location','NorthWest');
...
y = @(x) 2.5*exp(-(x(1)^2+x(2)^2)) + sin(2*x(1)) + cos(10*x(2));
...
N = 100;
P = N^2;
%s = linspace(min(min(xT))-0.5,max(max(xT))+0.5,N);
s = linspace(-5,5,N);
x = zeros(P,2); y = zeros(P,1);
count = 1;
for i = 1:N
...
hold on
surfc(s,s,y_rs,'EdgeColor','none','FaceAlpha',0.5)
view(20,20)
for i = 1:size(xT,1)
    for k = 1:i
        scatter3(xT(k,1),xT(k,2),yT(k),100,'MarkerEdgeColor',[0,0,0],'MarkerFaceColor',clr(k,:))
    end
    plot3(xT(1:i,1),xT(1:i,2),yT(1:i),'k-.')
    colormap(cmap)
    colorbar
    axis square
    pause(0.5)
